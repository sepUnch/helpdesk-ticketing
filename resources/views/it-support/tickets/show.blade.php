    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-4">
                <table class="table table-borderless">
                    ...
                    <tr><th>Dibuat Oleh</th><td>{{ $ticket->user->name }}</td></tr>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white py-3">
                <h6 class="fw-bold mb-0"><i class="bi bi-arrow-right-circle"></i> Update Status</h6>
            </div>
            <div class="card-body p-4">
                @php
                    $next = ['open'=>'on_progress','on_progress'=>'resolved','resolved'=>'closed'];
                    $nextStatus = $next[$ticket->status] ?? null;
                @endphp
                @if($nextStatus)
                <form method="POST" action="{{ route('it-support.tickets.update-status', $ticket) }}">
                    @csrf
                    <div class="mb-3">
                    ...
                    <button type="submit" class="btn btn-success">
                        ...
                        Update ke {{ strtoupper(str_replace('_', ' ', $nextStatus)) }}
                    </button>
                </form>
                @endif
            </div>
